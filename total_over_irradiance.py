...
def total_over_irradiance(raw_rad, dados, ghi, clear_sky, dia_final, mes, ano, arquivo, nome_var, nome_arquivo):
    ...
    ghi_avg = pd.to_numeric(raw_rad.iloc[:, ghi[0]])
    ghi_max = pd.to_numeric(raw_rad.iloc[:, ghi[0]])  # GHI 1 max
    ...
    clear_sky = pd.to_numeric(raw_rad.iloc[:, clear_sky[0]])
    n, m = raw_rad.shape
    ...
    # !
    if maior_evento_1000.size > 0 and np.nanmax(maior_evento_1000) > 0:
        max_maior_evento_1000 = np.nanmax(maior_evento_1000)
    else:
        max_maior_evento_1000 = 0
    ...
    total_xplot2(variavel1=over_irradiance_plot, variavel2=ioh, data=data, num_figura=1, 
                titulo=arquivo, dia_final=dia_final, mes=mes, ano=ano,
    ...
    total_xplot_over(variavel1=ioh, variavel2=clear_sky, variavel3=ghi_avg, data=data, num_figura=2,
                    titulo=arquivo, dia_final=dia_final, mes=mes, ano=ano,
                lim_sy=1800, lim_iy=0, und_y='W/m²', tam_font=10, var1='Ioh', var2='Clear',var3=nome_var,
                nome_arquivo=nome_arquivo)
    ...
    if n_eventos_x > 0:
        total_xplot_dia(variavel1=ioh, variavel2=clear_sky, variavel3=ghi_avg, 
                        data=data, num_figura=3, titulo='Day of the most intense event ', 
                        dia=dia_maior_evento, lim_sy=1800, lim_iy=0, und_y='W/m²', 
                        tam_font=15, var1='Ioh', var2='Clear', var3=nome_var, nome_arquivo=nome_arquivo)
        
        total_xplot_dia(variavel1=ioh, variavel2=clear_sky, variavel3=ghi_avg, 
                        data=data, num_figura=4, titulo='Day the most lasting event ', 
                        dia=dia_maior_duracao, lim_sy=1800, lim_iy=0, und_y='W/m²', 
                        tam_font=15, var1='Ioh', var2='Clear', var3=nome_var, nome_arquivo=nome_arquivo)
        
        total_xplot_dia(variavel1=ioh, variavel2=clear_sky, variavel3=ghi_avg, 
                        data=data, num_figura=5, titulo='Day of the event with the highest energy ', 
...
# Total de eventos
    ...
    # %%
    # AUXX e AUX
    auxx = [['Events lasting 1 minute'], ['Events lasting 2 minutes'], ['Events lasting 3 minutes'],
            ['Events lasting 4 minutes'], ['Events lasting 5 minutes'], ['Total number of events']]
    auxx = [auxx[i] + [p[i][j] for j in range(len(p[0]))] for i in range(len(auxx))]

    aux = [['Time', 'Total number of events', 'Number of events with GHI >= 1000 and < 1367 W/m²', 
            'Number of events with GHI >= 1000 and < 1367 W/m² (%)', 'Number of events with GHI > 1367 W/m²', 
            'Number of events with GHI > 1367 W/m² (%)', 'Average GHI value for events > 1000 and < 1367 W/m²', 
            'Maximum GHI value for events > 1000 and < 1367 W/m²', 'Average GHI value for events > 1367 W/m²', 
            'Maximum GHI value for events > 1367 W/m²', '', '', 'Event with the longest duration', 
            'Event with the longest duration > 1000 and < 1367', 'Event with the longest duration > 1367', 
            '', 'Hour/Day of largest event', 'Value of largest event in W/m²', 
            'Hour/Day of largest energy event', 'Energy value of largest event Wh/m²']]
    ...
    nome = f"{nome_arquivo}-{arquivo} - Eventos_info.xlsx"
    pd.DataFrame(df).to_excel(nome, index=False, header=None)
    ...
    AUX_concat = pd.concat([aux1, aux2, aux3, aux4], axis=1)
